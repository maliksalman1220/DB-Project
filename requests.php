...
<div class="text-center mt-0 pt-0 mx-3">	
	...
	<?php 
	echo "<a class='btn btn-primary m-1' href=profile.php?email=$useremail>Profile</a>";
	...
	echo "<a class='btn btn-primary m-1' href=dashboard.php?email=$useremail>Gigs</a>";
	echo "<a class='btn btn-primary m-1' href=requests.php?email=$useremail>Requests</a>";
    echo "<hr>";
	...
	{
		echo "<a href=requests.php?email=$useremail&catID=$row1[CatID]>$row1[Categories] </a>";
	}
	...
	{ 	
		$query = "SELECT * FROM `users` JOIN `requests` ON `users`.`UserID` = `requests`.`UserID` WHERE `requests`.`CatID` = '$catID'";
		$result = mysqli_query($conn, $query);
	}

	while ($row = mysqli_fetch_assoc($result))
	{
		$query1 = "SELECT * FROM `requests` JOIN `requestorders` ON `requests`.`RequestID` = `requestorders`.`RequestID` WHERE `requests`.`RequestID` = '$row[RequestID]'";
		$result1 = mysqli_query($conn, $query1);
		...
		if ($row1['Status'] == "")
		{
			...
			<h5 class='card-title'><a href=bids.php?email=$useremail&RequestID=$row[RequestID]>$row[Title]</a></h5>
			</div>";
		}
		else
		{
		...
		echo "
		  ...
		  <div class='card-footer text-muted'>
                   Expected Delivery Days: $row[RequiredDeliveryDays]
		  </div>
		</div>
		";
		if ($AccountType == "freelancer")
		{
			echo "<a class='btn btn-primary mt-3' href=postbid.php?email=$useremail&MyID=$MyID&UserID=$row[UserID]&RequestID=$row[RequestID]>Bid<a>";
		}
	}
